...
def get_dom(results, dom_id):
    ...
    print("dom_id = " + dom_id)
    print("APPLITOOLS_API_KEY_READ = " + str(os.getenv("APPLITOOLS_API_KEY_READ", None)))
    ...
    url = urlunsplit(
        urlsplit_app_session_url._replace(
            path="/api/images/dom/" + dom_id + "/", query = accountId + "&" + urlencode({"apiKey": "SsEfrro4C1h0AJBhWkjDG9ZIM5IPZRG8WWMhdR1EMU4110", "format": "json"})#({"apiKey": os.getenv("APPLITOOLS_API_KEY_READ", None), "format": "json"})
        )
    )
...
def getNodesByAttribute000(dom, attribute_name):
    ...
    jsonpath_expression = parse('$.childNodes[*].childNodes[*].childNodes[?(@.attributes.data-applitools-scroll=="true")]')#"data-applitools-scroll")]')
